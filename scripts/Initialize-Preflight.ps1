...
function Get-PreflightChecks {
    ...
    # 6. Windows ADK (oscdimg voor Build-UnattendedIsos.ps1)
    ...
    $results.Add([PSCustomObject]@{
        ...
        Detail  = if ($existingSwitch) { "vSwitch bestaat al" } else { "Wordt aangemaakt door Configure-HostNetwork.ps1" }
        Block   = $false
    })
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <Grid>
    ...
    <StackPanel Grid.Row="5" Orientation="Horizontal" HorizontalAlignment="Right" Margin="16,12,16,16">
      ...
      <Button x:Name="BtnNext" Content="Doorgaan naar Configure-HostNetwork →" Style="{StaticResource PrimaryBtn}"
              IsEnabled="False"/>
    </StackPanel>
  </Grid>
</Window>
...
$btnInstallHyperV.Add_Click({
    ...
    try {
        ...
        [System.Windows.MessageBox]::Show(
            "Hyper-V is geïnstalleerd.`n`nHerstart je laptop en voer daarna Initialize-Preflight.ps1 opnieuw uit.",
            "Herstart vereist",
