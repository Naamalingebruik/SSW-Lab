...
# ============================================================
# SSW-Lab | 06-SETUP-MGMT.ps1
# Installeert PowerShell-modules en RSAT-tools op LAB-MGMT01
# via PowerShell Direct (geen netwerk vereist, wel VM Running).
#
# GUI: kies traject (AZ-104 / MD-102 / MS-102 / SC-300 / Full)
#      vink modules aan/uit en start installatie.
# ============================================================
...
# ── Module-catalogus ──────────────────────────────────────────────────────────
# Elke module hoort bij één of meer trajecten.
# 'RSAT' = Windows Capability (geen PSGallery-module).
$catalog = @(
    [PSCustomObject]@{ Name = "Az";                       Certs = @("AZ-104","Full");                         Type = "PSGallery"; Desc = "Azure PowerShell - abonnementen, resources, VM's" }
    [PSCustomObject]@{ Name = "Microsoft.Graph";          Certs = @("AZ-104","MD-102","MS-102","SC-300","Full"); Type = "PSGallery"; Desc = "Microsoft Graph API - gebruikers, groepen, beleid" }
    [PSCustomObject]@{ Name = "ExchangeOnlineManagement"; Certs = @("MS-102","Full");                         Type = "PSGallery"; Desc = "Exchange Online beheer" }
    [PSCustomObject]@{ Name = "MicrosoftTeams";           Certs = @("MS-102","Full");                         Type = "PSGallery"; Desc = "Teams-beleid en configuratie" }
    [PSCustomObject]@{ Name = "PnP.PowerShell";           Certs = @("MS-102","Full");                         Type = "PSGallery"; Desc = "SharePoint Online en OneDrive beheer" }
    [PSCustomObject]@{ Name = "WindowsAutoPilotIntune";   Certs = @("MD-102","Full");                         Type = "PSGallery"; Desc = "Autopilot-apparaatregistratie via Intune" }
    [PSCustomObject]@{ Name = "PSWindowsUpdate";          Certs = @("MD-102","Full");                         Type = "PSGallery"; Desc = "Windows Update beheer op afstand" }
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <Grid Margin="24">
    ...
    <StackPanel Grid.Row="0" Margin="0,0,0,16">
      ...
      <TextBlock Text="Selecteer een certificeringstraject — modules worden automatisch aangevinkt" Style="{StaticResource Sub}"/>
    </StackPanel>

    <!-- Traject knoppen -->
    <Border Grid.Row="1" Background="#313244" CornerRadius="6" Padding="16,12" Margin="0,0,0,12">
      <StackPanel>
        <TextBlock Text="Certificeringstraject" Foreground="#A6ADC8" FontSize="11" Margin="0,0,0,10"/>
        <WrapPanel>
          ...
          <RadioButton x:Name="RadioFull"   Content="Full (alles)"  Style="{StaticResource TrajectBtn}" GroupName="Traject" IsChecked="True"/>
        </WrapPanel>
      </StackPanel>
    </Border>
    ...
    <Border Grid.Row="2" Background="#313244" CornerRadius="6" Padding="16,12" Margin="0,0,0,12">
      <StackPanel>
        <Grid Margin="0,0,0,8">
          ...
          <TextBlock Text="Modules en RSAT-tools" Foreground="#A6ADC8" FontSize="11" VerticalAlignment="Center"/>
          <StackPanel Grid.Column="1" Orientation="Horizontal">
            <Button x:Name="BtnSelectAll"   Content="Alles aan"  Style="{StaticResource SecBtn}" Width="90" Margin="0,0,6,0" Padding="8,0"/>
            <Button x:Name="BtnSelectNone"  Content="Alles uit"  Style="{StaticResource SecBtn}" Width="90" Padding="8,0"/>
          </StackPanel>
        </Grid>
    ...
    <Border Grid.Row="3" Background="#181825" CornerRadius="6" Margin="0,0,0,12">
      <ScrollViewer x:Name="LogScroll" VerticalScrollBarVisibility="Auto" Padding="12,8">
        <TextBlock x:Name="LogBox" Foreground="#A6ADC8" FontFamily="Consolas" FontSize="11"
                   TextWrapping="Wrap" Text="Klaar om te starten. Selecteer een traject en klik op Installeren."/>
      </ScrollViewer>
    </Border>
    ...
    <Grid Grid.Row="5">
      ...
      <Button x:Name="BtnClose"    Content="Sluiten"    Grid.Column="0" Style="{StaticResource SecBtn}"/>
      <Button x:Name="BtnInstall"  Content="Installeren" Grid.Column="2" Style="{StaticResource PrimaryBtn}"/>
    </Grid>
  </Grid>
</Window>
"@
...
foreach ($m in $catalog) {
    $obj = [PSCustomObject]@{
        ...
        Type    = $m.Type
        Certs   = $m.Certs
        Checked = $true   # Full standaard
    }
    # Voeg INotifyPropertyChanged-gedrag toe via hashtable-wrapper
    $moduleItems.Add($obj)
}
...
$btnInstall.Add_Click({
    ...
    if (-not $selected) {
        [System.Windows.MessageBox]::Show("Selecteer minimaal één module.", "Geen selectie", "OK", "Warning") | Out-Null
        return
    }
    ...
    if (-not $vm -or $vm.State -ne "Running") {
        [System.Windows.MessageBox]::Show("VM '$vmName' is niet beschikbaar of niet Running.", "VM fout", "OK", "Error") | Out-Null
        return
    }
    ...
    # Credential opvragen — normaliseer gebruikersnaam naar .\user of DOMAIN\user
    $cred = $null
    try {
    ...
    # Normaliseer: "administrator" (zonder prefix) -> ".\administrator"
    $user = $cred.UserName
    ...
    # Verbindingstest vooraf zodat een fout direct zichtbaar is
    try {
        ...
        $window.Dispatcher.Invoke([action]{
            [System.Windows.MessageBox]::Show(
                "Kan niet verbinden met $vmName.`n`nGebruikersnaam: $($cred.UserName)`nFout: $errMsg`n`nGebruik LAB\Administrator of .\Administrator.",
                "Verbinding mislukt", "OK", "Error"
            ) | Out-Null
    ...
    # Map RSAT-weergavenamen naar Capability-namen
    $rsatMap = @{
    ...
    $total = 3  # stap 1 prep, stap 2 modules, stap 3 RSAT
    $step  = 0

    $job = [System.Threading.Thread]::new([System.Threading.ThreadStart]{
        try {
            # ── Stap 1: NuGet + PSGallery ────────────────────────────────
            Add-Log "=== Stap 1/3 — NuGet en PSGallery ===" "#89B4FA"
            ...
            # ── Stap 2: PSGallery modules ────────────────────────────────
            Add-Log "=== Stap 2/3 — PSGallery modules ($($psModules.Count)) ===" "#89B4FA"
                ...
                Invoke-Command -VMName $vmName -Credential $cred -ArgumentList (,$psModules) -ErrorAction Stop -ScriptBlock {
                    ...
                    foreach ($name in $names) {
                        ...
                        if ($inst) {
                            ...
                            } catch { "  SKIP     $name  (versiecheck mislukt)" }
                        } else {
                            try {
                    ...
                    $color = if ($_ -match "^  FOUT") { "#F38BA8" } elseif ($_ -match "UPDATED|INSTALLED") { "#A6E3A1" } else { "#A6ADC8" }
                    Add-Log $_ $color
                }
            } else { Add-Log "  Geen PSGallery-modules geselecteerd — overgeslagen." "#6C7086" }

            $step++
            $window.Dispatcher.Invoke([action]{ $progress.Value = ($step / $total) * 100 })

            # ── Stap 3: RSAT ─────────────────────────────────────────────
            Add-Log "=== Stap 3/3 — RSAT Windows-features ($($rsatCaps.Count)) ===" "#89B4FA"
            if ($rsatCaps.Count -gt 0) {
                Invoke-Command -VMName $vmName -Credential $cred -ArgumentList (,$rsatCaps) -ErrorAction Stop -ScriptBlock {
                    ...
                    $color = if ($_ -match "^  FOUT") { "#F38BA8" } elseif ($_ -match "INSTALLED") { "#A6E3A1" } else { "#A6ADC8" }
                    Add-Log $_ $color
                }
            } else { Add-Log "  Geen RSAT-features geselecteerd — overgeslagen." "#6C7086" }

            $step++
            $window.Dispatcher.Invoke([action]{ $progress.Value = 100 })
            Add-Log "" 
            Add-Log "Klaar! Setup MGMT01 voltooid." "#A6E3A1"

        } catch {
            Add-Log "FOUT: $($_.Exception.Message)" "#F38BA8"
        } finally {
            $window.Dispatcher.Invoke([action]{
                $btnInstall.IsEnabled = $true
                $btnClose.IsEnabled   = $true
