...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
  ...
  <Grid Margin="24">
    ...
    <StackPanel Grid.Row="7" Orientation="Horizontal" HorizontalAlignment="Right" Margin="0,12,0,0">
      ...
      <Button x:Name="BtnNext"   Content="Doorgaan naar 04-SETUP-DC →" Style="{StaticResource Btn}"
...
$btnCreate.Add_Click({
    ...
    foreach ($key in $sel) {
        ...
        if ($existing) {
            ...
            if (-not $isDry) {
                try {
                    if (-not $isoPath -or -not (Test-Path $isoPath)) { Write-Log "ISO niet gevonden voor $vmName."; continue }
    ...
    $progress.Value = 100
    Write-Log $(if ($isDry) { "✔ Dry Run klaar — niets aangemaakt." } else { "✔ Klaar. Start de VMs om de installatie te voltooien." })
...
$btnNext.Add_Click({
    $reader.Close()
    $next = Join-Path $PSScriptRoot "04-SETUP-DC.ps1"
