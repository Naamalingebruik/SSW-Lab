  ...
  <Grid Margin="24">
    ...
    <StackPanel Grid.Row="1" Margin="0,0,0,8">
      ...
      <TextBlock Foreground="#CDD6F4" FontSize="12" TextWrapping="Wrap" Margin="0,4,0,0">
        ...
        <LineBreak/><Run Text="2. Manual: maak een Team site aan in SharePoint admin center"/>
        <LineBreak/><Run Text="3. Manual: configureer externe delingsinstellingen"/>
        <LineBreak/><Run Text="4. Teams PowerShell: Teams en leden opvragen"/>
        <LineBreak/><Run Text="5. Manual: Teams Meetings policy aanpassen"/>
        <LineBreak/><Run Text="6. Manual: bekijk Teams usage reports in M365 admin center"/>
      </TextBlock>
    </StackPanel>
...
$btnRun.Add_Click({
    ...
    Write-LabLog "${pre}Stap 5: Manual — Teams Meetings policy"
    $progress.Value = 70
...
$btnNext.Add_Click({
    ...
    else { [System.Windows.MessageBox]::Show("lab-week6-defender.ps1 not found.", "SSW-Lab") }
    $reader.Close()
})
