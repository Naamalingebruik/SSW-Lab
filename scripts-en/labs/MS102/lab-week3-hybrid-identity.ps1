...
$btnRun.Add_Click({
    ...
    Write-LabLog "${pre}Stap 3: Manual — MFA configureren via Entra admin center"
    $progress.Value = 44
    ...
    # ── Stap 5: Sign-in logs ─────────────────────────────────
    Write-LabLog "${pre}Stap 5: Manual — Sign-in logs analyseren"
    $progress.Value = 72
...
$btnNext.Add_Click({
    ...
    else { [System.Windows.MessageBox]::Show("lab-week4-exchange.ps1 not found.", "SSW-Lab") }
    $reader.Close()
})
