...
$btnRun.Add_Click({
    ...
    Write-Log "${pre}Stap 4: Manual — Enterprise app en consent bekijken"
    $progress.Value = 68
    ...
    # ── Stap 5: App Proxy ────────────────────────────────────
    Write-Log "${pre}Stap 5: Manual — App Proxy connector bekijken"
    $progress.Value = 84
...
$btnNext.Add_Click({
    ...
    else { [System.Windows.MessageBox]::Show("lab-week6-governance-pim.ps1 not found.", "SSW-Lab") }
    $reader.Close()
})
