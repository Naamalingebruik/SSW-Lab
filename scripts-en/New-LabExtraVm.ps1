...
if ($resolvedMemoryGB -le 0 -or $resolvedDiskGB -le 0 -or $resolvedCpuCount -le 0) {
    throw "CPU, RAM, and disk must be greater than 0."
}
...
if (Test-Path -LiteralPath $diskPath) {
    throw "Disk file already exists: $diskPath"
}
...
Write-ExtraVmLog "New VM  : $VmName"
Write-ExtraVmLog "Path     : $resolvedVmRoot"
